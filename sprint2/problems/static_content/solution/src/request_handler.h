...
private:
    ...
    ResponseVariant HandleStatic(const Request& req) {
        ...
        std::string decoded_target = UrlDecode(target);// /images/cube.svg
        fs::path rel_path{decoded_target.substr(1)};// images/cube.svg
        fs::path full_path = fs::weakly_canonical(path_to_static_ / rel_path); // static/images/cube.svg
