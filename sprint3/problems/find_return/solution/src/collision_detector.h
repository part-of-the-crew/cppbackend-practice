...
// CHANGE 1: Use geom::Position (double) instead of Point2D (int)
CollectionResult TryCollectPoint(geom::Position a, geom::Position b, geom::Position c);

struct Item {
    geom::Position position;  // <--- Changed from Point2D
    double width;
};

struct Gatherer {
    geom::Position start_pos;  // <--- Changed from Point2D
    geom::Position end_pos;    // <--- Changed from Point2D
    double width;
};
