...
// Provider updated to use geom::Position (double) instead of Point2D (int)
class TestProvider : public collision_detector::ItemGathererProvider {
public:
    TestProvider() = default;

    // Changed parameter to geom::Position
    void AddItem(geom::Position pos, double width) { items_.push_back({pos, width}); }

    // Changed parameters to geom::Position
    void AddGatherer(geom::Position start, geom::Position end, double width) {
...
TEST_CASE("Gatherer moves directly through item", "[collision]") {
    ...
    // Position now accepts doubles
    provider.AddItem({5.0, 0.0}, 0.0);

    // Gatherer moves from (0, 0) to (10, 0)
    provider.AddGatherer({0.0, 0.0}, {10.0, 0.0}, 0.0);
...
TEST_CASE("Gatherer collects multiple items", "[collision]") {
    ...
    provider.AddItem({2.0, 0.0}, 0.5);  // 1st item
    provider.AddItem({8.0, 0.0}, 0.5);  // 2nd item

    provider.AddGatherer({0.0, 0.0}, {10.0, 0.0}, 0.5);
